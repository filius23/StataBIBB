...
```{stata  reg_anova4, echo =F , collectcode = F}
set linesize 200
qui use "D:\Datenspeicher\BIBB_BAuA/BIBBBAuA_2018_suf1.0_clean.dta"
reg F518_SUF i.m1202
```
...
+ Befragte mit dualer o. schulischer Berufsausbildung/einf.,mittl. Beamte verdienen im Schnitt  586.0038 EUR mehr als Befragte ohne Abschluss (`m1202 = 1`). Der Unterschied ist aber statistisch signifikant (`P>|t|` < 0,05).
+ Befragte mit  Aufstiegsfortbildung verdienen im Schnitt 1483.236 EUR mehr als Befragte ohne Abschluss (`m1202 = 1`). Der Unterschied ist aber statistisch signifikant (`P>|t|` < 0,05).
+ Befragte mit Abschluss einer Fachhochschule, Universität/ geh., höhere Beamte verdienen im Schnitt 2493.142 EUR mehr als Befragte ohne Abschluss (`m1202 = 1`). Der Unterschied ist aber statistisch signifikant (`P>|t|` < 0,05).
