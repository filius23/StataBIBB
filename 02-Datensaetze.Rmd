...
+ S.18 *ff.* in [Diaz-Bone, R. (2019). Statistik für Soziologen (4. Auflzpalter)](https://www.utb-studi-e-book.de/9783838550718)
+ S.12 *ff.* in [Bortz, J., & Schuster, C. (2010). Statistik für Human- und Sozialwissenschaftler (7. Auflzpalter)](https://link.springer.com/book/10.1007/978-3-642-12770-0) (ausführliche formale Beschreibung)
...
Manchmal möchten wir nur bestimmte Beobachtungen im Datensatz behalten. Beispielsweise möchten wir für eine Analyse nur Befragte, die jünger als 60 Jahre sind, im Datensatz behalten. Dazu können wir `keep` oder `drop` die gewünschten Fälle behalten bzw. die ungewünschten ausschließen:
```{stata keep, eval = F}
keep if zpalter < 60
```
...
(@) Die Variable `gkpol` beinhaltet die Größe der Wohngemeinde der Befragten.
      ...
      + Lassen Sie sich eine Auzählung der Wohnortgröße lediglich für die weiblichen Befragten ausgeben (Geschlecht ist Variable `S1` erfasst). Denken Sie an die Unterscheidung zwischen Labels und Zahlenwerten.
...
(@) Die Variable `F1104` beinhaltet das Jahr des Schulabschlusses.
      ...
      + Sie wollen nun die Jahre für den Schulabschluss für ausschließlich die weiblichen Befragten ansehen - nutzen Sie passende `if`-Bedingung! (Geschlecht ist in Variable `S1` abgelegt)
